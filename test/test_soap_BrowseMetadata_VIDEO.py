...
try:
    ...
    try:
        ...
        response_xml = open("./dut_xml/browse_BrowseMetadata_VIDEO.xml", "w")
        response_xml.write(response.read())
        response_xml.close()
        diff = main.diff_files('ref_xml/browse_BrowseMetadata_VIDEO.xml', 'dut_xml/browse_BrowseMetadata_VIDEO.xml')
        if len(diff) is 0:
            print '\nTEST PASSED\n' 
        else:
            print '\nTEST FAILED\n'
