...
try:
    ...
    try:
        ...
        response_xml = open("./dut_xml/browse_BrowseMetadata_root.xml", "w")
        response_xml.write(response.read())
        response_xml.close()
        diff = main.diff_files('ref_xml/browse_BrowseMetadata_root.xml', 'dut_xml/browse_BrowseMetadata_root.xml')
        if len(diff) is 0:
            print '\nTEST PASSED\n' 
        else:
            print '\nTEST FAILED\n'
